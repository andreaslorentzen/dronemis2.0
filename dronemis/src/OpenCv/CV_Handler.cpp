...
void CV_Handler::paintCube(Point center, std::string type) {
    ...
    if (!type.compare("Green")) {
        circle(map, cv::Point(navData->getPosition().x, navData->getPosition().y), 7, Scalar(0, 255, 0), thickness, lineType);
        imwrite(output_map_name, map);
    }
    else if (!type.compare("Red")){
        circle(map, cv::Point(navData->getPosition().x, navData->getPosition().y), 7, Scalar(0, 0, 255), thickness, lineType);
        imwrite(output_map_name, map);
    }
    imshow("MapMis", map);
    waitKey(10);
